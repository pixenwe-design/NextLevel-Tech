...
import {ArrowLeft,ArrowRight,ChevronDown,PackageCheck,Search,ShoppingCart,SlidersHorizontal,X} from "lucide-react";
...
const matchCategory=(slug:string,p:Product,mainCategories:MainCategory[])=>{
 if(slug==="todas")return true;
 const name=categoryList.find(x=>x[0]===slug)?.[1]||slug;
 return categoryMatchesSelection(p.category,name,mainCategories);
};
export default function CategoryCatalog({slug,initial}:{slug:string,initial:Record<string,string>}){
 ...
 const [query]=useState(initial.q||"");const [brand,setBrand]=useState(initial.marca||"todas");const [min,setMin]=useState(initial.min||"");const [max,setMax]=useState(initial.max||"");
 ...
 const [sort,setSort]=useState(initial.orden||"recientes");const [filtersOpen,setFiltersOpen]=useState(false);
 ...
 const base=storeProducts.filter(p=>matchCategory(slug,p,mainCategories));const brands=[...new Set(base.map(p=>p.brand))];const brandLabel=(name:string)=>slug==="consolas"?(name==="Sony"?"PlayStation":name==="Microsoft"?"Xbox":name):name;
 ...
 return <div className="categoryPage"><main className="categoryMain"><section className="categoryHero"><img className="categoryHeroImage" src={categoryImages[slug]||categoryImages.todas} alt=""/><div className="categoryHeroShade"/><div className="categoryHeroContent"><div className="categoryHeroTop"><span className="breadcrumb"><Link href="/">Inicio</Link><i>/</i><span>Categorías</span><i>/</i><b>{title}</b></span><Link href="/" className="categoryBack"><ArrowLeft/> Volver al inicio</Link></div><h1>{title}</h1><p>{descriptions[slug]||`Explorá nuestra selección de ${title.toLowerCase()} con garantía y soporte local.`}</p><strong>{visible.length} {visible.length===1?"producto encontrado":"productos encontrados"}</strong></div></section>
