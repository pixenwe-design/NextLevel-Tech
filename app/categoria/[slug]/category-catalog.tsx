...
function Card({p}:{p:Product}){return <article className="product"><div className="pic">{p.oldPrice&&<span className="badge">-{Math.round((1-p.price/p.oldPrice)*100)}%</span>}<img src={p.image} alt={p.name}/></div><div className="pcopy"><span className="pbrand">{p.brand} · {p.category}</span><h3>{p.name}</h3><span className="instock"><PackageCheck/>{p.stock?`${p.stock} disponibles`:"Agotado"}</span><div className="pprice"><b>{gs(p.price)}</b>{p.oldPrice&&<del>{gs(p.oldPrice)}</del>}</div><div className="actions"><button>Ver detalles <ArrowRight/></button><button><ShoppingCart/></button></div></div></article>}
