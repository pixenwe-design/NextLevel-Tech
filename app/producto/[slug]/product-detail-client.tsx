...
export default function ProductDetail({slug}:{slug:string}){
 ...
 return <>
  ...
  <main className="detail section">
   ...
   <div className="relatedCarousel"><div className="products related">{related.map(item=><article className="product" key={identity(item)}><div className="pic"><img src={item.image} alt={item.name}/></div><div className="pcopy"><span className="pbrand">{item.brand} · {item.category}</span><h3><Link href={`/producto/${productSlug(item)}`}>{item.name}</Link></h3><span className="instock"><PackageCheck/>{item.stock?`${item.stock} disponibles`:"Agotado"}</span><div className="pprice"><b>{gs(item.price)}</b>{item.oldPrice&&<del>{gs(item.oldPrice)}</del>}</div><div className="actions"><Link href={`/producto/${productSlug(item)}`}>Ver detalles <ArrowRight/></Link><button onClick={()=>addItem(item)} disabled={!item.stock} aria-label={`Agregar ${item.name} al carrito`}><ShoppingCart/></button></div></div></article>)}</div><span className="relatedSwipeHint" aria-hidden="true">›</span></div>
  </main>
 </>
}
